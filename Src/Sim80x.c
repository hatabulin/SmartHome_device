...
void	Sim80x_Init(osPriority Priority)
{
  ...
  char answer[25];
  snprintf(answer, sizeof(answer),"Sim80xTask created...\n\r");
...
void  Sim80x_BufferProcess(void)
{
  ...
  HAL_UART_Transmit(&huart1, (uint8_t*)strStart,strlen(strStart), HAL_MAX_DELAY);
//  UpdateGridBox(answer);
...
{ 
  ...
  uint8_t UnreadMsgCounter=1;
	...
	char answer[25];
	snprintf(answer, sizeof(answer),"Sim80xTask running\n\r");
...
*/
    ...
    //###########################################
    if(HAL_GetTick() - TimeForSlowRun > 30000)
    {
